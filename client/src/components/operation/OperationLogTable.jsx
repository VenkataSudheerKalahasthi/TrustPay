export function OperationLogTable({ operations = [] }) {
  if (operations.length === 0) {
    return <p className="text-xs text-surface-400 p-4 text-center">No system operations logged yet.</p>;
  }
        ...
        <tbody className="divide-y divide-surface-800 text-xs">
          {operations.map((op) => (
            <tr key={op.id} className="hover:bg-surface-850 transition-colors">
              <td className="p-3.5 font-bold text-surface-100">{op.name}</td>
              <td className="p-3.5 text-3xs font-mono uppercase text-surface-400">{op.category}</td>
              <td className="p-3.5">
