...
export function CreateCapacityPlanModal({ isOpen, onClose, onSuccess }) {
  const [name, setName] = useState('');
  const [organizationId] = useState('org_default_01');
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [targetCapacityHours, setTargetCapacityHours] = useState(160);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await workforceService.createCapacityPlan({
        organizationId,
        name,
        startDate,
        endDate,
        targetCapacityHours: Number(targetCapacityHours),
      });
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div>
            ...
            <input
              type="text"
              required
              placeholder="e.g. Q3 Enterprise Workforce Capacity"
              value={name}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Target Capacity Hours</label>
            <input
              type="number"
              required
              min="1"
              value={targetCapacityHours}
              onChange={(e) => setTargetCapacityHours(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-sky-500"
            />
          </div>
