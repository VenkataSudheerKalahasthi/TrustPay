...
export function ConversationList({
  ...
  return (
    <div className="w-full md:w-80 lg:w-96 bg-surface-900 border-r border-surface-800 flex flex-col h-full shrink-0">
      {/* Header */}
      <div className="p-4 border-b border-surface-800 space-y-3 shrink-0">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-surface-800 border border-surface-700 text-xs text-surface-100 placeholder-surface-500 focus:outline-none focus:border-primary-500"
          />
        </div>
      </div>
            ...
            return (
              <button
                ...
                className={`w-full p-4 flex items-start gap-3 text-left transition-colors relative ${
                  isActive ? 'bg-primary-500/10 border-l-4 border-primary-500' : 'hover:bg-surface-800/40'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1 mb-1">
                    <h4 className="text-xs font-semibold text-surface-100 truncate">{title}</h4>
                    {lastMessage && (
                  ...
                  <div className="flex items-center gap-1 text-2xs text-surface-400 mb-1">
                    <span className="font-mono text-3xs font-semibold text-primary-400 bg-primary-500/10 px-1.5 py-0.2 rounded">
                  ...
                  {lastMessage && (
                    <p className="text-xs text-surface-400 truncate leading-relaxed">
                      {lastMessage.content}
                    </p>
                  )}
                </div>
