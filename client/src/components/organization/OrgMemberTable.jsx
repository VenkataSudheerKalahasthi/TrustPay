...
export function OrgMemberTable({ members = [], onUpdateRole, onRemoveMember }) {
  if (members.length === 0) {
    return <p className="text-xs text-surface-600 p-4 text-center">No members in this organization yet.</p>;
  }
        ...
        <tbody className="divide-y divide-surface-800 text-xs">
          {members.map((m) => (
            <tr key={m.id} className="hover:bg-surface-850 transition-colors">
              <td className="p-3.5 font-semibold text-surface-900">
                {m.user?.firstName} {m.user?.lastName}
                <span className="block text-3xs font-mono font-normal text-surface-600">{m.user?.email}</span>
              </td>
              <td className="p-3.5">
                <select
                  value={m.role}
                  onChange={(e) => onUpdateRole(m.userId, e.target.value)}
                  className="bg-surface-50 border border-surface-300 text-xs text-surface-900 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-600"
                >
              ...
              <td className="p-3.5 text-right">
                <button
                  ...
                  className="p-1.5 rounded-lg text-surface-600 hover:text-red-400 hover:bg-surface-50 transition-colors"
                  title="Remove Member"
                >
                  <Trash2 size={14} />
                </button>
