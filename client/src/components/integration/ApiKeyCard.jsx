    ...
    <div className="p-4 rounded-2xl bg-surface-900 border border-surface-800 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-xs font-bold text-surface-100">{apiKey.name}</h4>
            <span className="text-3xs font-mono text-surface-400">Prefix: {apiKey.keyPrefix}...</span>
          </div>
        </div>

        <button
          onClick={() => onRevoke(apiKey.id)}
          className="p-2 rounded-xl text-surface-400 hover:text-red-400 hover:bg-surface-800 transition-colors"
      ...
      {rawKey && (
        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 space-y-2">
          ...
          <code className="block text-xs font-mono bg-surface-950 p-2 rounded text-surface-100 break-all select-all">
            {rawKey}
          </code>
        </div>
      )}
