...
export function WalletBalanceCard({ title, amount, type = 'available', currency = 'INR' }) {
  const getIcon = () => {
    switch (type) {
      ...
      case 'refunded':
        return <ArrowDownLeft size={18} className="text-info-400" />;
      default:
  ...
  const getBorderColor = () => {
    switch (type) {
      ...
      default:
        return 'border-surface-800 bg-surface-900/60';
    }
  };

  return (
    <div className={`p-5 rounded-2xl border ${getBorderColor()} flex flex-col justify-between`}>
      <div className="flex items-center justify-between gap-2 mb-3">
        <span className="text-2xs font-bold text-surface-400 uppercase tracking-wider">
          {title}
        </span>
        <div className="p-2 rounded-xl bg-surface-900/80 border border-surface-800">
          {getIcon()}
        </div>
      </div>

      <div>
        <div className="text-2xl font-bold text-surface-100 font-display">
          ₹{(amount || 0).toLocaleString('en-IN')}
        </div>
        <span className="text-2xs text-surface-500 font-mono">{currency}</span>
      </div>
    </div>
  );
}
