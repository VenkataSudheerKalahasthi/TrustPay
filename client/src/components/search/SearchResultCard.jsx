...
export function SearchResultCard({ result, onResultClick }) {
  ...
  return (
    <div className="p-4 rounded-xl bg-card border border-surface-200 hover:border-surface-300 transition-all flex items-start gap-3">
      <div className="p-2.5 rounded-xl bg-primary-50 border border-primary-600/30 text-primary-600 shrink-0 mt-0.5">
        <Icon size={16} />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-1">
          <Link
            ...
            className="text-sm font-bold text-surface-900 hover:text-primary-600 transition-colors truncate"
            dangerouslySetInnerHTML={{ __html: result.highlightedTitle || result.title }}
          />
          <span className="text-3xs font-mono uppercase font-bold px-2 py-0.5 rounded bg-surface-50 text-primary-600 shrink-0">
            {result.entityType}
          </span>
        </div>
        ...
        {result.content && (
          <p
            className="text-xs text-surface-700 line-clamp-2 leading-relaxed mb-2"
