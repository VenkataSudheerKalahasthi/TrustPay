export function TimelineItem({ title, description, timestamp, icon: Icon, isLast = false }) {
  return (
    <div className="flex gap-4 relative">
      ...
      <div className="w-8 h-8 rounded-full bg-surface-50 border border-surface-300 flex items-center justify-center text-primary-600 shrink-0 z-10">
      ...
      <div className="flex-1 pb-6">
        ...
        {description && <p className="text-xs text-surface-600 mt-1 leading-relaxed">{description}</p>}
      </div>
    </div>
  );
}
