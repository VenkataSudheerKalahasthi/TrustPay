  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <Card className="p-4 bg-card border-surface-200 space-y-3">
        ...
        <div className="space-y-1.5">
          {pinnedItems.length === 0 ? (
            <p className="text-3xs text-surface-600">No pinned items.</p>
          ) : (
            pinnedItems.map((item) => (
              <Link
                key={item.id}
                to={item.linkUrl}
                className="flex items-center justify-between p-2 rounded-xl bg-surface-100/60 hover:bg-surface-50 text-xs font-semibold text-surface-800 transition-colors"
              >
                <span className="truncate">{item.title}</span>
                <ExternalLink size={10} className="text-surface-600 shrink-0" />
              </Link>
            ))
          )}
        </div>
      ...
      <Card className="p-4 bg-card border-surface-200 space-y-3">
        ...
        <div className="space-y-1.5">
          {bookmarks.length === 0 ? (
            <p className="text-3xs text-surface-600">No bookmarks saved.</p>
          ) : (
            bookmarks.map((bm) => (
              <Link
                key={bm.id}
      ...
      <Card className="p-4 bg-card border-surface-200 space-y-3">
        ...
        <div className="space-y-1.5">
          {recentlyViewed.length === 0 ? (
            <p className="text-3xs text-surface-600">No recent history.</p>
          ) : (
