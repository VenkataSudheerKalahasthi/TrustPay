...
export function SecurityDashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {loading ? (
        <p className="text-xs text-surface-600">Loading security intelligence...</p>
      ) : (
        <>
          ...
          {data?.incidents && data.incidents.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-xs font-bold text-surface-800 uppercase tracking-wider">Security Incidents</h3>
            ...
            <Card className="p-4 bg-card border-surface-200 space-y-3">
              ...
              <div className="space-y-1.5">
                {data?.activeSessions?.map((s) => (
                  <div key={s.id} className="flex items-center justify-between p-2 rounded-xl bg-surface-100/60 text-xs font-semibold text-surface-800">
                    <div>
                      <span>Session ID: {s.sessionId.substring(0, 10)}...</span>
                      <span className="block text-3xs font-mono text-surface-600">IP: {s.ipAddress || '127.0.0.1'}</span>
                    </div>
                    <button
                      onClick={async () => {
            ...
            <Card className="p-4 bg-card border-surface-200 space-y-3">
              ...
              <div className="space-y-1.5">
                {data?.trustedDevices?.map((d) => (
                  <div key={d.id} className="flex items-center justify-between p-2 rounded-xl bg-surface-100/60 text-xs font-semibold text-surface-800">
