...
export function TalentPoolsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <form onSubmit={handleCreatePool} className="p-4 rounded-2xl bg-card border border-surface-200 space-y-3">
        <h3 className="text-xs font-bold text-surface-900">Create New Talent Pool Folder</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          <p className="text-xs text-surface-600">Loading talent pools...</p>
        ) : pools.length === 0 ? (
          <p className="text-xs text-surface-600">No talent pools created yet.</p>
        ) : (
          pools.map((p) => (
