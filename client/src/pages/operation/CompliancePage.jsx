  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-surface-200 uppercase tracking-wider">My GDPR Data Export History</h3>
        <div className="space-y-2">
          {exportRequests.map((req) => (
            <div key={req.id} className="p-4 rounded-2xl bg-surface-900 border border-surface-800 flex items-center justify-between">
              <div className="space-y-1">
                <span className="text-xs font-bold text-surface-100">Data Export Package (#{req.id.substring(0, 8)})</span>
                <span className="block text-3xs font-mono text-surface-400">Requested: {new Date(req.requestedAt).toLocaleString()}</span>
              </div>

              {req.downloadUrl && (
                <a href={req.downloadUrl} target="_blank" rel="noreferrer" className="p-2 rounded-xl bg-surface-800 text-primary-400 hover:bg-surface-700 transition-colors">
                  <Download size={16} />
                </a>
              )}
            </div>
      ...
      <div className="space-y-3 pt-4">
        <h3 className="text-xs font-bold text-surface-200 uppercase tracking-wider">System Compliance Reports</h3>
