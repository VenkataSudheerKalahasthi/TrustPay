...
export function ActivityCenterPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 p-2 bg-surface-900 border border-surface-800 rounded-2xl overflow-x-auto text-xs scrollbar-hide">
        {[
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-16 rounded-xl bg-surface-900 border border-surface-800 animate-pulse" />
          ))}
        </div>
      ) : error ? (
