        ...
        <div className="glass-card overflow-hidden relative">
          ...
          <div className="p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 -mt-16 sm:-mt-20 relative z-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-5">
              <div className="p-1 rounded-3xl bg-surface-950 shadow-glow">
                <Avatar name={name} src={user.avatar} size="xl" status="online" />
              </div>
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-bold font-display text-surface-50">{name}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {profile.educations && profile.educations.length > 0 && (
                <div className="flex flex-col gap-3">
                  ...
                  {profile.educations.map((edu) => (
                    <div key={edu.id} className="text-xs text-surface-300 border-l-2 border-surface-800 pl-3 py-1">
                      <p className="font-semibold text-surface-100">{edu.degree} in {edu.fieldOfStudy}</p>
                      <p className="text-2xs text-surface-400">{edu.institution}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="glass-card p-6 flex flex-col gap-4">
              <h3 className="text-sm font-bold text-surface-100 font-display">Specialist Rates</h3>
              <div className="flex items-center justify-between border-b border-surface-800/80 pb-3">
                <span className="text-xs text-surface-400">Hourly Rate</span>
            ...
            {/* Verified Skills */}
            <div className="glass-card p-6 flex flex-col gap-3">
              <h3 className="text-sm font-bold text-surface-100 font-display">Verified Skills</h3>
              {skills.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skills.map((s, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg bg-surface-800 text-surface-200 text-xs font-medium border border-surface-700/60"
                    >
                      {s}
                    </span>
                  ))}
